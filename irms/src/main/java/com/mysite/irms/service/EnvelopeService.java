...
public class EnvelopeService {
	...
	// 파일을 AES 암호화, AES 키를 RSA 공개키로 암호화, 전자서명 저장
	public void saveEncryptedEnvelope(MultipartFile file, String username) throws Exception {
		...
		if (originalFileName == null || !(originalFileName.endsWith(".log") || originalFileName.endsWith(".txt") || originalFileName.endsWith(".LOG"))) {
			throw new RuntimeException("허용되지 않은 파일 형식입니다. .log 또는 .txt 파일만 업로드할 수 있습니다.");
		}
	...
	public String verifyAndDecryptEnvelope(Long envelopeId, String username) throws Exception {
		...
		KeyPairEntity userKeyPair = keyRepository.findByUser(user)
				.orElseThrow(() -> new RuntimeException("키쌍이 존재하지 않습니다."));
		...
		// 파일 복호화를 위한 개인키
		PrivateKey privateKey = RSAUtil.readPrivateKeyFromPem(userKeyPair.getPrivateKey());
		...
		// 업로더의 공개키
		User uploader = userRepository.findByUsername(envelope.getUploaderUsername())
				.orElseThrow(() -> new RuntimeException("업로더 계정 정보를 찾을 수 없습니다."));
		KeyPairEntity uploaderKey = keyRepository.findByUser(uploader)
				.orElseThrow(() -> new RuntimeException("업로더 키쌍이 존재하지 않습니다."));
		PublicKey uploaderPublicKey = RSAUtil.readPublicKeyFromPem(uploaderKey.getPublicKey());

		boolean isValid = RSAUtil.verifySignature(encryptedContent, envelope.getSignature(), uploaderPublicKey);
		if (!isValid) {
	...
	public void transferEnvelope(EnvelopeTransferRequest request, String senderUsername) {
		...
		User sender = userRepository.findByUsername(senderUsername)
				.orElseThrow(() -> new RuntimeException("보내는 사용자 조회 실패"));

		KeyPairEntity senderKeys = keyRepository.findByUser(sender)
				.orElseThrow(() -> new RuntimeException("보내는 사용자의 키쌍 없음"));
		KeyPairEntity receiverKeys = keyRepository.findByUser(receiver)
				.orElseThrow(() -> new RuntimeException("수신자의 키쌍 없음"));

		try {
			// sender 개인키 사용
			PrivateKey senderPrivateKey = RSAUtil.readPrivateKeyFromPem(senderKeys.getPrivateKey());
			byte[] aesKeyBytes = RSAUtil
					.decryptWithPrivateKey(Base64.getDecoder().decode(envelope.getAesKeyEncrypted()), senderPrivateKey);
			String encodedAESKey = new String(aesKeyBytes); // base64로 인코딩된 AES 키

			// 수신자 공개키로 AES 키 재암호화
			PublicKey receiverPublicKey = RSAUtil.readPublicKeyFromPem(receiverKeys.getPublicKey());
			String newEncryptedAESKey = RSAUtil.encryptWithPublicKey(encodedAESKey, receiverPublicKey);

			// AES 키 암호화적용
			envelope.setReceiverUsername(receiver.getUsername());
			envelope.setAesKeyEncrypted(newEncryptedAESKey);

			envelopeRepository.save(envelope);
		} catch (Exception e) {
			throw new RuntimeException("봉투 전달 중 암호화 오류 발생: " + e.getMessage());
		}
	}
